window.tab = "	";
window.collapsedImage = "/oschina/collapsed.gif";
window.expandedImg = "/oschina/expanded.gif";
...
function Process(addQuote) {
	...
	try {
		if (json == "")
			json = "\"\"";
		var obj = eval("[" + json + "]");
		html = ProcessObject(obj[0], 0, false, false, false, addQuote);
...
function CollapsibleViewClicked() {
	if ($("#CollapsibleView").val()) {
		$("#CollapsibleViewDetail").show();
	}
	$("#CollapsibleViewDetail").hide();
	Process();
}
...
function MakeContentVisible(element, visible) {
	...
	if (!!img.tagName && img.tagName.toLowerCase() == "img") {
		...
		element.previousSibling.firstChild.src = visible ? window.expandedImg
				: window.collapsedImage;
	}
...
function ExpImgClicked(img) {
	...
	if (!container)
		return;
	var disp = "none";
	var src = window.collapsedImage;
	if (container.style.display == "none") {
		disp = "inline";
		src = window.expandedImg;
	}
...
function TabSizeChanged(tabSize) {
	if (tabSize == "tab") {
		window.tab = "	";
	} else {
		window.tab = MultiplyString(parseInt(tabSize), window.tab);
	}
	Process();
}
...
function EnsureIsPopulated() {
	...
	if (!$("#Canvas").innerHTML && !!$("#RawJson").value)
		Process();
...
function LinkToJson() {
	...
	var val = $("#RawJson").value;

	val = escape(val.split('/n').join(' ').split('/r').join(' '));

	$("#InvisibleLinkUrl").value = val;

	$("#InvisibleLink").submit();
